...
local function can_client_format(client)
    ...
    if conf.lsp.servers[client.name] == true or client.name == 'null-ls' then
        return true
    end

    -- check config server settings
    if conf.lsp.servers[client.name] then
        -- default to true if no format flag on server settings is set
        if conf.lsp.servers[client.name].format == nil then
            return true
        end

        -- check format flag on server settings
        return (conf.lsp.servers[client.name].format == true)
    end
...
function M.format(bufnr)
    vim.lsp.buf.format({
        timeout_ms = conf.lsp.format_timeout,
