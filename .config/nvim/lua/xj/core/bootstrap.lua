--- ### XXXNvim Core Bootstrap
--
-- This module simply sets up the global `xj` module.
-- This is automatically loaded and should not be resourced, everything is accessible through the global `xj` variable.
--
-- @module xj.core.bootstrap
-- @copyright 2022
...
xj.install = _G["xj_installation"] or { home = vim.fn.stdpath "config" }
xj.supported_configs = { xj.install.home }
--- external xj configuration folder
xj.install.config = vim.fn.stdpath("config"):gsub("[^/\\]+$", "xj")
-- check if they are the same, protects against NVIM_APPNAME use for isolated install
if xj.install.home ~= xj.install.config then
  vim.opt.rtp:append(xj.install.config)
  --- supported xj user conifg folders
  table.insert(xj.supported_configs, xj.install.config)
end
...
local function load_module_file(module)
  ...
  for _, config_path in ipairs(xj.supported_configs) do
...
function xj.user_opts(module, default, extend)
...
xj.updater = {
  options = xj.user_opts("updater", { remote = "origin", channel = "stable" }),
  snapshot = { module = "lazy_snapshot", path = vim.fn.stdpath "config" .. "/lua/lazy_snapshot.lua" },
  rollback_file = vim.fn.stdpath "cache" .. "/xj_rollback.lua",
}
local options = xj.updater.options
if xj.install.is_stable ~= nil then options.channel = xj.install.is_stable and "stable" or "nightly" end
