--- ### XXXNvim LSP Utils
--
-- LSP related utility functions to use within XXXNvim and user configurations.
--
-- This module can be loaded with `local lsp_utils = require("xj.core.utils.lsp")`
--
-- @module xj.core.utils.lsp
-- @see xj.core.utils
-- @copyright 2022
...
M.setup_diagnostics = function(signs)
  local default_diagnostics = xj.user_opts("diagnostics", {
    virtual_text = true,
    signs = { active = signs },
...
M.setup = function(server)
  ...
  if not vim.tbl_contains(xj.lsp.skip_setup, server) and setup_handler then setup_handler(server, opts) end
end
...
--- The `on_attach` function used by XXXNvim
...
M.on_attach = function(client, bufnr)
  ...
  for id, _ in pairs(xj.lsp.progress) do -- clear lingering progress messages
    if not next(vim.lsp.get_active_clients { id = tonumber(id:match "^%d+") }) then xj.lsp.progress[id] = nil end
  end
...
--- The default XXXNvim LSP capabilities
  ...
  if server_name == "lua_ls" then -- by default initialize neodev and disable third party checking
    ...
    lsp_opts.before_init = function(param, config)
      if vim.b.neodev_enabled then
        for _, xj_config in ipairs(xj.supported_configs) do
          if param.rootPath:match(xj_config) then
            table.insert(config.settings.Lua.workspace.library, xj.install.home .. "/lua")
            break
          end
        end
      end
