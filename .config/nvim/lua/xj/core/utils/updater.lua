--- ### XXXNvim Updater
--
-- XXXNvim Updater utilities to use within XXXNvim and user configurations.
--
-- This module can also loaded with `local updater = require("xj.core.utils.updater")`
--
-- @module xj.core.utils.updater
-- @see xj.core.utils
-- @copyright 2022
-- @license GNU General Public License v3.0

local git = require "xj.core.utils.git"
...
--- Helper function to generate XXXNvim snapshots (For internal use only)
...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(xj.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(xj.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
--- Get the current XXXNvim version
---@param quiet? boolean Whether to quietly execute or send a notification
---@return string # The current XXXNvim version string
...
--- Get the full XXXNvim changelog
---@param quiet? boolean Whether to quietly execute or display the changelog
---@return table # The current XXXNvim changelog table of commit messages
...
function M.update_packages()
  ...
  require("xj.core.utils.mason").update_all()
end

--- Create a table of options for the currently installed XXXNvim version
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(xj.updater.rollback_file, "w"))
...
--- XXXNvim's rollback to saved previous version function
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, xj.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then opts = xj.updater.options end
  opts = require("xj.core.utils").extend_tbl({ remote = "origin" }, opts)
...
--- XXXNvim's updater function
---@param opts? table the settings to use for the update
function M.update(opts)
  if not opts then opts = xj.updater.options end
  opts = require("xj.core.utils").extend_tbl(
  ...
  then
    ...
    local summary = {
      { "XXXNvim updated successfully to ", "Title" },
      { git.current_version(), "String" },
      { "!\n", "Title" },
      {
        opts.auto_quit and "XXXNvim will now update plugins and quit.\n\n"
    ...
    -- if the user wants to auto quit, create an autocommand to quit XXXNvim on the update completing
    if opts.auto_quit then
      vim.api.nvim_create_autocmd("User", {
        desc = "Auto quit XXXNvim after update completes",
        pattern = "ZXUpdateComplete",
        command = "quitall",
      })
    end
