--- ### XXXNvim Buffer Utilities
--
-- Buffer management related utility functions
--
-- This module can be loaded with `local buffer_utils = require "xj.core.utils.buffer"`
--
-- @module xj.core.utils.buffer
-- @copyright 2022
...
function M.move(n)
  ...
  require("xj.core.utils").event "BufsUpdated"
...
function M.close(bufnr, force)
  ...
  if require("xj.core.utils").is_available "bufdelete.nvim" then
...
---@param compare_func string|function a string of a comparator defined in require("xj.core.utils.buffer").comparator or a custom comparator function
---@param skip_autocmd boolean|nil whether or not to skip triggering ZXBufsUpdated autocmd event
...
function M.sort(compare_func, skip_autocmd)
  ...
  if type(compare_func) == "function" then
    ...
    vim.t.bufs = bufs
    if not skip_autocmd then require("xj.core.utils").event "BufsUpdated" end
...
local function unique_path(bufnr)
  return require("xj.core.utils.status.provider").unique_path() { bufnr = bufnr }
