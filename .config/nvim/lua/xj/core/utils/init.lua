--- ### XXXNvim Utilities
--
-- Various utility functions to use within XXXNvim and user configurations.
--
-- This module can be loaded with `local utils = require "xj.core.utils"`
--
-- @module xj.core.utils
-- @copyright 2022
...
--- Partially reload XXXNvim user settings. Includes core vim options, mappings, and highlights. This is an experimental feature and may lead to instabilities until restart.
...
function M.reload(quiet)
  ...
  local core_modules = { "xj.core.bootstrap", "xj.config.options", "xj.config.mappings" }
  ...
  if not quiet then -- if not quiet, then notify of result
    if success then
      M.notify("XXXNvim successfully reloaded", vim.log.levels.INFO)
    else
      M.notify("Error reloading XXXNvim...", vim.log.levels.ERROR)
    end
  end
...
--- Get an icon from the XXXNvim internal icons if it is available and return it
---@param kind string The kind of icon in xj.icons to retrieve
...
function M.get_icon(kind, padding, no_fallback)
  ...
  if not M[icon_pack] then
    M.icons = xj.user_opts("icons", require "xj.core.icons.nerd_font")
    M.text_icons = xj.user_opts("text_icons", require "xj.core.icons.text")
  end
  local icon = M[icon_pack] and M[icon_pack][kind]
  return icon and icon .. string.rep(" ", padding or 0) or ""
end

--- Get a icon spinner table if it is available in the XXXNvim icons. Icons in format `kind1`,`kind2`, `kind3`, ...
...
--- Serve a notification with a title of XXXNvim
---@param msg string The notification body
---@param type number|nil The type of the notification (:help vim.log.levels)
---@param opts? table The nvim-notify options to use (:help notify-options)
function M.notify(msg, type, opts)
  vim.schedule(function() vim.notify(msg, type, M.extend_tbl({ title = "XXXNvim" }, opts)) end)
end

--- Trigger an XXXNvim user event
---@param event string The event name to be appended to ZX
function M.event(event)
  vim.schedule(function() vim.api.nvim_exec_autocmds("User", { pattern = "ZX" .. event, modeline = false }) end)
end
...
function M.alpha_button(sc, txt)
  ...
  local sc_ = sc:gsub("%s", ""):gsub("LDR", "<leader>"):gsub("LOL","<localleader>")
  ...
  -- if the localleader is set, replace the text with the actual localleader key for nicer printing
  if vim.g.mapleader then sc = sc:gsub("LOL", vim.g.maplocalleader == "\\" and "BSL" or vim.g.maplocalleader) end
