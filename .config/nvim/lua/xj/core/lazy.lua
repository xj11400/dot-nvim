...
if not vim.loop.fs_stat(lazypath) then
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      require("xj.core.utils").notify "Mason is installing packages if configured, check status with :Mason"
    end,
  })
end
vim.opt.rtp:prepend(lazypath)

local user_plugins = xj.user_opts "plugins"
for _, config_dir in ipairs(xj.supported_configs) do
...
require("lazy").setup(xj.user_opts("lazy", {
  spec = spec,
  defaults = { lazy = true },
